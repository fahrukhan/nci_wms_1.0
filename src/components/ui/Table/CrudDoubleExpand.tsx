...
export const TableExpand = <T extends object>({
  ...
  const getSubData = (rowId: number, refId: string) => {
    return supportData.filter((item: any) => item[refId] === rowId);
  };
        ...
        >
          ...
          >
            ...
            {page.map((row) => {
              ...
              return (
                <React.Fragment key={`row-${row.original.id}`}>
                  <tr {...rowProps} className="transition hover:bg-gray-100">
                    ...
                    {row.cells.map((cell: any) => {
                      ...
                      return (
                        <td
                          key={`cell-${row.original.id}-${cell.column.id}`} // Ensure each cell has a unique key
                          {...cellProps}
                    ...
                    <tr key={`expanded-${row.id}`}>
                      <td colSpan={columns.length + 1} className="px-6">
                        <table className="min-w-full bg-gray-100 rounded">
                          ...
                          <tbody>
                            {getSubData(row.original.id, "gudang_id").map(
                              (subItem: any) => (
                                <tr key={`sub-${subItem.id}`}>
                                  <td
                                    key={`name-${subItem.id}`}
                                  ...
                                  <td
                                    key={`created_at-${subItem.id}`}
                                    className="px-6 py-4 text-sm font-medium text-gray-900"
                                  >
                                    {subItem.created_at}
                                  </td>
                                  <td
                                    key={`updated_at-${subItem.id}`}
                                    className="px-6 py-4 text-sm font-medium text-gray-900"
                                  >
                                    {subItem.updated_at}
                                  </td>
