...
import { EditUnitComponent } from "./EditUnitComponent";
import { Unit } from "@/drizzle/schema/MasterData/masterData.schema";
...
export const UnitTable = ({
  ...
  const { sortedData, requestSort } = useSortableData<Unit>(units);

  const fetchData = useCallback(
    async (page: number, search: string) => {
      try {
        const { data, pagination } = await fetchUnits(
          page,
  ...
  const unitColumns = [
    ...
    { header: "Symbol", key: "symbol", width: 30 },
  ];
  const exportUnits = () => {
    exportToExcel(fetchUnits, unitColumns, "Unit", pagination.totalItems);
  };
